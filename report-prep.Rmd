...
for (k in 1:nrow(folds)) {
  ...
  LL.rmse = sum(sqrt((test.y - as.matrix(cbind(1 * rep(1, nrow(test.X)), test.X)) %*% LL.coefs)^2))
  LL.rmses = cbind(LL.rmses, LL.rmse)
  
  NR.rmse = sum(sqrt((test.y - as.matrix(cbind(1 * rep(1, nrow(test.X)), test.X)) %*% NR.coefs)^2))
  NR.rmses = cbind(NR.rmses, NR.rmse)
  }
```

```{r, eval = FALSE}
a = tibble(
  `Logistic LASSO` = c(LL.rmses),
  `Newton-Raphson` = c(NR.rmses)
) %>% 
